...
# Common path
strPath = 'N:\\1. Micro Evaluation\\VELOCITY\\paragraph_predict\\'

##### PART 1 Use pre-trained model #####
# load and clean data
# lesson_raw = pd.read_excel(strPath + 'text and categories.xlsx')
lesson_raw = pd.read_csv(strPath + 'text and categories.csv', encoding = "ISO-8859-1")
...
def prepare_documents(folderPath=os.path.join(os.getcwd())):
        ...
        if file.endswith(".docx"):
            try:
                ...
                for index, paragraph in enumerate(document.paragraphs):
                    ...
                    # clean data(save sentences whose length is greater than 40)
                    if paragraph.text not in '' and len(temp_word_list) >= 25:
...
##################
# It's only for testing, comment this out(put # before) when running the program normally

    # print("The shape of the training dtm is: ", X_train_dtm.shape)
    # print("The shape of the training data is: ", X_train)
    # print("The shape of the original data is ", X_train)
    # print("The shape of the training class is", Y_train)
...
if Model == 'rf':
    ...
    # save the prediction
    if Category.equals(Y1):
        forest_df_result.to_excel(os.path.join('tables', 'rf_prediction_c8_p3.xlsx'))
    else:
        forest_df_result.to_excel(os.path.join('tables', 'rf_prediction_c46_p3.xlsx'))

    print('Finish prediction with Random Forest Model !')
    # return forest_score
...
if Model == "nn":
    ...
    # get parameters
    # number of neurons for a layer
    # num_input_neurons = X_train_dtm.shape[1]
    # num_output_neurons = len(Y_train.unique())
    # num_sample_size = X_train_dtm.shape[0]
    # scale_factor = 2
    # num_hidden_neurons = round(num_sample_size/(scale_factor * (num_input_neurons + num_output_neurons)))
...
df_universe = pd.read_excel(os.path.join('tables', 'universe_prediction.xlsx'), sheetname='frequency')
...
# df_result_wide = df_result.pivot(index='project_id', columns='category', values='salient')


# calculate most salient value within group(group by topics)
# df_result_topic = df_master.groupby('category').apply(lambda x: x[x.salient>=1.5*x.salient.mean()])\
#     .drop('category', axis=1)
# df_result_topic = df_result_topic.reset_index(level=0)
# # order by project_id
# df_result_topic = df_result_topic.groupby('category')\
#     .apply(lambda x: x.sort_values('salient', ascending=False)).drop('category', axis=1).reset_index(level=0)


# convert to wide format
# df_result_topic_wide = df_result_topic.pivot(index='category', columns='project_id', values='salient')
